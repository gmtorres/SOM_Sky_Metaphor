...
OBJECTS_CLASSES = [ComponentPlane, HitHist, UMatrix, DMatrix, UStar_PMatrix, 
                   SDH, PieChart, NeighbourhoodGraph, Chessboard, Clustering, MetroMap, QError, TimeSeries, SkyMetaphor]
...
class SOMToolbox():
    ...
    def __init__(self, m, n, dimension, weights, input_data=None, classes=None, component_names=None):
        ...
        self._visualizations[13]._activate_controllers()
    ...
    def _interpolation(self, ):
        if self._maincontrol.interpolation:
            self._pipe.send(resize(self._plot, (2000, 2000)))
        else:
