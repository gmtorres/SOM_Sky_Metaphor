...
OBJECTS_ALL = {'Component Planes': 0, 'Hit Histogram': 1, 'U-matrix': 2, 'D-Matrix': 3,  
               ...
               'Quantization Error': 11,  'Time Series': 12, 'SkyMetaphor': 13} 
...
SkyMetaphorPallete = ['gist_gray', 'cividis', 'Blues', 'copper', 'Greys']
class SkyMetaphorController(param.Parameterized):
    neighbors            = param.Integer(3, bounds=(0, 4), label='Number of neighbors')
    lbd            = param.Number(0.25, bounds=(0.0, 0.5), step=0.01, label='Lambda')
    back         = param.Boolean(True, label='Activate back')

    palette = param.ObjectSelector(default='gist_gray', objects=SkyMetaphorPallete, label='Pallet')
    background = param.ObjectSelector(default='SHD', objects=['Hist', 'UMatrix', 'SHD'], label='Background')
    points_color = param.ObjectSelector(default='white', objects=['black', 'white', 'blue'], label='Points color')
    high_def     = param.Boolean(False, label='High Definition')
    definition_scale    = param.Integer(200, bounds=(20, 1000), label='Definition quality')
    sdh_factor    = param.Integer(3, bounds=(1, 25), label='SDH Factor')
    ...
    @param.depends("lbd", watch=True)
    def _change_lbd(self,):
        self._calculate(True)

    @param.depends("neighbors", watch=True)
    def _change_neighbors(self,):
        self._calculate(True)

    @param.depends("back", watch=True)
    def _change_back(self,):
        self._calculate(True, True)

    @param.depends("palette", watch=True)
    ...
    @param.depends("high_def", watch=True)
    def _change_definition(self,):
        self._calculate(True, True)

    @param.depends("points_color", watch=True)
    def _change_point_color(self,):
        self._calculate(True, False)
    
    @param.depends("definition_scale", watch=True)
    def _change_definition_scale(self,):
        self._calculate(True, True)
        
    @param.depends("sdh_factor", watch=True)
    def _change_sdh_factor(self,):
        self._calculate(True, True)
